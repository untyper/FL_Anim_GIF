...
bool Fl_Anim_GIF::load(char *buf_, int len_, bool from_file_ /* = false*/) {
  ...
  if (len_ < 6 || buf_[0] !='G' || buf_[1] !='I' || buf_[2] != 'F') {
    Fl::error("Fl_GIF_Image is not a GIF file.\n");
    if (from_file_) {
      free(buf_);
    }
    return false;
  }
  if (buf_[3]!='8' || buf_[4] >'9' || buf_[5] != 'a') {
    Fl::warning("GIF version is %c%c%c.", buf_[3], buf_[4], buf_[5]);
    memcpy(&buf_[3], "89a", 3); // make gif_load happy
  }
  ...
  if (!_valid) {
    Fl::error("Fl_Anim_GIF has invalid format.\n");
  }
  return _valid;
} // load
